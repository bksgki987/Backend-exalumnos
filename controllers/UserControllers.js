...
module.exports = {
  create: async (req, res) => {
    ...
    try {
      ...
      if (userExists) res.status(400).json({ mesage: 'Email already taken' });
  ...
  updateOne: async (req, res) => {
    ...
    try {
      ...
      if (!user) res.staus(404).json({ message: 'User not found' });
  ...
  deleteOne: async (req, res) => {
    ...
    try {
      ...
      await UserService.deliteOneById(id);
      res.status(204).json();
    } catch (err) {
      res.status(400).json(err);
    }
  ...
  login: async (req, res) => {
    ...
    try {
      ...
      if (!user) res.status(400).json({ mesage: 'error on validation' });
      // eslint-disable-next-line no-unused-vars
      const isValid = comparePasswords(user.password, password);
      if (!isValid) res.status(400).json({ mesage: 'Password dont match' });
